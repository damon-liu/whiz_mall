package com.damon.common.service.impl;
...
import com.damon.common.exception.IdempotencyException;
 ...
 */
public class SuperServiceImpl<M extends BaseMapper<T>, T> extends ServiceImpl<BaseMapper<T>, T> implements ISuperService<T> {
    ...
    public boolean saveIdempotency(T entity, DistributedLock locker, String lockKey, Wrapper<T> countWrapper, String msg) throws Exception{
        ...
        if (StrUtil.isEmpty(lockKey)) {
            throw new LockException("分布式锁key为空！");
        }
        try (
            ...
            if (lock != null) {
                //判断记录是否已存在
                int count = ((Long) super.count(countWrapper)).intValue();
                if (count == 0) {
